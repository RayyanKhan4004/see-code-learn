  ...
  head: () => ({
    meta: [
      { title: "CodeVision — See how your code actually runs" },
      {
        name: "description",
        content:
          "Interactive visualizer for the JavaScript call stack, heap, event loop, and queues. Built for developers who want to understand, not just memorize.",
      },
    ],
  }),
  component: Landing,
    ...
    <div className="h-full overflow-auto">
      <section className="relative mx-auto flex max-w-5xl flex-col items-center px-6 py-20 text-center">
        ...
        <p className="mt-5 max-w-2xl text-base text-muted-foreground md:text-lg">
          CodeVision animates the call stack, memory heap, event loop, and queues step-by-step — so
          you can finally see <em>why</em> async code behaves the way it does.
        </p>
          ...
          {[
            {
              ...
              color: "stack",
              title: "Call Stack",
              body: "Watch frames push and pop in real time as functions execute.",
            },
            {
              icon: Cpu,
              color: "heap",
              title: "Heap & GC",
              body: "See objects allocated, referenced, and garbage-collected.",
            },
            {
              icon: Zap,
              color: "microtask",
              title: "Event Loop",
              body: "Microtasks, macrotasks, Web APIs — finally make sense.",
            },
          ].map((f) => (
            <div key={f.title} className="rounded-xl border border-border bg-card p-5 text-left">
              <div
                ...
                style={{
                  background: `color-mix(in oklab, var(--${f.color}) 18%, transparent)`,
                  color: `var(--${f.color})`,
                }}
              >
