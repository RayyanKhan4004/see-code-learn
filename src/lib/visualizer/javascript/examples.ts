...
const variables: ExampleProgram<JsStep> = {
  ...
  trace: [
    ...
    alloc("u1", "user { id, role }", "object", 3, "Allocating object on the heap", "Objects are reference types — the variable holds a pointer.", "{ id: 1, role: 'admin' }"),
    push("console.log", 4, "Calling console.log(user)", "Function invocation pushes a frame onto the call stack."),
    log("{ id: 1, role: 'admin' }", 4, "Logging the object", "console.log reads from the heap via the reference stored in `user`."),
...
const functions: ExampleProgram<JsStep> = {
  ...
  trace: [
    push("(script)", 1, "Global execution context created", "The script itself runs in the global frame.", "Execution Context"),
    push("console.log", 6, "Evaluating arguments to console.log", "Arguments must be evaluated before the call."),
    push("square(5)", 6, "Calling square(5)", "A new frame is pushed for square."),
    push("multiply(5, 5)", 5, "square calls multiply(5, 5)", "Nested calls stack on top — LIFO order."),
...
const closures: ExampleProgram<JsStep> = {
  ...
  trace: [
    push("makeCounter", 8, "Calling makeCounter()", "Creates a new execution context.", "Execution Context"),
    alloc("env1", "[[Scope]] { count: 0 }", "closure", 2, "Allocating closure environment", "The inner function captures this environment.", "count = 0"),
    alloc("fn1", "counter (closure)", "function", 3, "Returning inner function", "The function keeps a reference to its lexical environment.", "() => { count++; return count; }"),
    ...
    log("2", 10, "count becomes 2, returns 2", "State persists across calls — the magic of closures."),
...
const hoisting: ExampleProgram<JsStep> = {
  ...
  trace: [
    ...
    alloc("greetFn", "greet()", "function", 4, "Function `greet` hoisted fully", "Function declarations are available before their textual position.", 'function greet() { return "hi"; }'),
    push("console.log", 1, "Logging typeof greet", "Function is already in scope."),
    log('"function"', 1, "typeof greet is 'function'", "Because `greet` was hoisted with its body."),
    pop(1, "console.log returns", ""),
    push("console.log", 2, "Logging x", "`var x` was hoisted but only the declaration, not the value."),
    log("undefined", 2, "x is undefined", "var declarations default to undefined until assignment runs."),
...
const eventLoop: ExampleProgram<JsStep> = {
  ...
  trace: [
    ...
    pop(1, "Returns", ""),
    push("setTimeout", 2, "Calling setTimeout", "setTimeout itself is a synchronous call to a Web API."),
    {
    ...
    push("(anonymous)", 2, "Callback pushed onto the stack", "Now it executes like a normal function."),
...
const asyncAwait: ExampleProgram<JsStep> = {
  ...
  trace: [
    push("load", 6, "Calling load()", "Async functions run synchronously until the first await.", "Call Stack"),
...
const memory: ExampleProgram<JsStep> = {
  ...
  trace: [
    { kind: "note", line: 1, explanation: "a = 10 stored by value", why: "Primitives are copied by value.", concept: "Stack" },
    { kind: "note", line: 2, explanation: "b = a — value 10 copied", why: "Changing b later wouldn't affect a.", concept: "Stack" },
    alloc("o1", "{ value: 1 }", "object", 3, "Object allocated on heap", "`obj` holds a pointer to it.", "{ value: 1 }"),
    { kind: "note", line: 4, explanation: "ref = obj — pointer copied", why: "Both variables point to the SAME heap object.", concept: "Heap" },
    { kind: "note", line: 5, explanation: "ref.value = 99 mutates the heap object", why: "Because obj and ref share the same reference.", concept: "Heap" },
...
const gc: ExampleProgram<JsStep> = {
  ...
  trace: [
    alloc("u1", "{ name: 'Ada' }", "object", 1, "Allocate user object on heap", "Reference held by `user`.", "{ name: 'Ada' }"),
    {
      kind: "note",
      line: 2,
